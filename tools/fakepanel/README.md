...
| The whole line's guide from `xmltv.php`: **Test News HD** with the same programmes as its short guide, **Sport One** — which has no short guide — with a match on now, a channel no stream uses, an entity, CDATA, a second title in French, and a programme with an unreadable start | The full guide: kept for the line's two channels only (the third dropped, the bad programme skipped), and **Sport One** showing "Live: Rovers & United" as now with "Match of the Evening" next — which only the full guide can give it |
...
| **Two audio tracks**: a live stream carrying English and Italian audio, each named in the PMT's ISO 639 descriptor | The audio-and-subtitles panel on a channel: the CC button appearing at all, the languages being read rather than "Audio 1", and a choice switching the audio mid-stream |
| Film **Two audio tracks and subtitles (MKV)**: English and French audio and an English subtitle track | The same panel on a film, the Subtitles section with Off, and the choice being remembered — leave, reopen, and the subtitles come back on. Subtitles are on the film rather than the channel because a live TS carries DVB subtitles, which are bitmaps, and ffmpeg cannot make those from text |
| **Old 4:3 channel**: 640x480, where everything else here is 16:9 | The picture-shape setting — Fit leaves it in pillarbox bars, Fill crops it to the screen, Stretch widens it. Nothing else on this panel tells those three apart, because every other stream and every screen the app runs on is 16:9 |
| **Test News HD** keeps four days of catch-up, and its guide has a programme that finished six hours ago and one from five days ago | Catch-up end to end: the finished one is offered with "Watch from the start", the five-day-old one is not, because it is outside the window |
| **Test Pattern TV** claims an archive of zero days | `hasCatchUp` — a panel promising nothing must not produce a button that always fails |
| `/timeshift/` streams what is inside four days and 404s what is outside, logging how long ago each request was for | That the start sent is the hour the viewer asked for. A client sending its own clock to a panel on another one gets television, just not the right television — the log is where that shows |
| `FAKE_GUIDE=narrow` serves an `xmltv.php` with nothing that has already been on | That the guide keeps the past it has already seen. Fetch once normally, then restart with this and refresh: the programmes from the first download must still be there for a channel with catch-up, and gone for one without |
